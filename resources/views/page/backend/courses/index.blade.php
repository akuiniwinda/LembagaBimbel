...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 d-flex align-items-stretch">
            <div class="card w-100">
                <div class="card-body p-4">
                    <h5 class="card-title fw-semibold mb-4">Recent Popular Courses</h5>
                    <div class="table-responsive">
                        <table class="table text-nowrap mb-0 align-middle">
                            <thead class="text-dark fs-4">
                                <tr>
                            ...
                            <tbody>
                                @foreach ($popularCourses as $kursuse)
                                    <tr>
                                        <td class="border-bottom-0">
                                            <img src="{{ asset('storage/' . $kursuse->photo) }}" width="100">
                                        </td>
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="form-check form-switch float-left custom-switch">
                                                    <input class="form-check-input" type="checkbox"
                                                        id="is_active_{{ $kursuse->id }}" name="is_active"
                                                        {{ $kursuse->is_active === 'active' ? 'checked' : '' }}
                                                        data-id="{{ $kursuse->id }}">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="border-bottom-0">
                                            <div class="d-flex align-items-center gap-2">
                                                <a class="badge bg-success rounded-3 fw-semibold"
                                                    href="/adminpanel/popularcourses/edit/{{ $kursuse->id }}">Edit</a>
                                                <a class="badge bg-danger rounded-3 fw-semibold"
                                                    href="/adminpanel/popularcourses/delete/{{ $kursuse->id }}">Delete</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <a href="/adminpanel/popularcourses/create" class="btn btn-elearning m-1 btn-custom">Tambah data</a>
    </div>
    <script>
