                            ...
                            <tr>
                                ...
                                <div class="d-flex align-items-center gap-2" >
                                    <div class="form-check form-switch float-left custom-switch">
                                        <input class="form-check-input" type="checkbox" id="is_active_{{$kontak->id}}" name="is_active" {{ $kontak->is_active === 'active' ? 'checked' : '' }} data-id="{{ $kontak->id }}">
                                    </div>
                                </div>
                                </td>
                                <td class="border-bottom-0">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll(".toggle-active").forEach(toggle => {
            toggle.addEventListener("change", function() {
                let contactId = this.dataset.id;  // Asumsi: data-id="${testimoni.id}" di HTML input
                let is_active = this.checked ? 1 : 0;  // 1 untuk active (checked), 0 untuk inactive

                fetch(`/adminpanel/contact/toggle-active/${contactId}`, {  // Perbaiki: Tambah backtick untuk template literal
                    method: "POST",
                    headers: {
