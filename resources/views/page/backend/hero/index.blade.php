                                            ...
                                            <td class="border-bottom-0">
                                                <div class="d-flex align-items-center gap-2">
                                                    <div class="form-check form-switch float-left custom-switch">
                                                        ...
                                                        type="checkbox"
                                                        id="is_active_{{ $hiro->id }}"
                                                        name="is_active"
                                                        {{ $hiro->is_active === 'active' ? 'checked' : '' }}
                                                        data-id="{{ $hiro->id }}"
                                                        >
                                                    </div>
                                                </div>
                                            ...
                                            <td class="border-bottom-0">
                                                <div class="d-flex align-items-center gap-2">
                                                    <a class="badge bg-success rounded-3 fw-semibold"
                                                        href="/adminpanel/hero/edit/{{ $hiro->id }}">Edit</a>
                                                    <a class="badge bg-danger rounded-3 fw-semibold"
                                                        href="/adminpanel/hero/delete/{{ $hiro->id }}">Delete</a>
                                                </div>
                                            </td>
                                        </tr>
                                        </tr>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".toggle-active").forEach(toggle => {
                toggle.addEventListener("change", function() {
                    let heroId = this.dataset.id; // Asumsi: data-id="${testimoni.id}" di HTML input
                    let is_active = this.checked ? 1 :
                    0; // 1 untuk active (checked), 0 untuk inactive

                    fetch(`/adminpanel/hero/toggle-active/${heroId}`, { // Perbaiki: Tambah backtick untuk template literal
                            method: "POST",
                            headers: {
